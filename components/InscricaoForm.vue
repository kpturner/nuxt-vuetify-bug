  ...
  <div
    ...
    <transition name="fade">
      <div
        ...
        <SuccessIcon />
        <h2 class="text-4xl text-center"> Parabéns, sua inscrição foi realizada! </h2>
      </div>
    </transition>
    <transition name="fade">
      <form
        ...
        action="post"
        class="p-8"
        @submit="postInscricao">
        <div class="container">
          <h2 class="text-3xl pb-3">{{ title }}</h2>
...
export default {
  ...
  methods: {
    async postInscricao (event) {
      event.preventDefault();
      let relation = (this.type === 'curso') ? {curso: this.id} : {workshop: this.id};
      let data = {...this.formData, ...relation};
      var rs = await this.$axios.post('/inscricaos', data)
